...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20']:
            ...
            backbone = resnet18(index_dim=p['index_dim'])
        ...
        elif p['train_db_name'] == 'coil20':
            from models.resnet_cifar import resnet18
            backbone = resnet18(index_dim=p['index_dim'])

        elif p['train_db_name'] == 'Dogs_vs_Cats':
            from models.resnet_cifar import resnet18
            backbone = resnet18(index_dim=p['index_dim'])
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        dataset = CIFAR10(train=True, transform=transform, download=False)
        # dataset[index] = {'image': img, 'target': target, 'meta': {'im_size': img_size, 'index': index, 'class_name': class_name}}
        # val_dataset = CIFAR10(train=False, transform=transform, download=False)
        ...
        from data.coil20 import COIL20
        dataset = COIL20(transform=transform)

    elif p['train_db_name'] == "Dogs_vs_Cats":
        from data.dogs_vs_cats import Dogs_vs_cats
        dataset = Dogs_vs_cats(transform=transform)
...
def get_train_dataloader(p, dataset):
    return torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'], batch_size=p['batch_size'], pin_memory=True, collate_fn=collate_custom, drop_last=False, shuffle=True)
# collate_custom 即用于collate的function，用于整理数据的函数。

def get_train_dataloader_coil20(p, dataset):
...
def get_train_transformations(p):
    ...
    if p['augmentation_strategy'] == 'standard___':
        # Standard augmentation strategy
        return transforms.Compose([
