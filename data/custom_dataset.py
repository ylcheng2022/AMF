...
class End2EndDataset_test(Dataset):
    def __init__(self, dataset, indices, num_neighbors=None):
        super(End2EndDataset_test, self).__init__()
        transform = dataset.transform
    ...
    def __getitem__(self, index):
        output = {}
        anchor = self.dataset.__getitem__(index)
        # anchor = {'image': img, 'target': target, 'meta': {'im_size': img_size, 'index': index, 'class_name': class_name}}
        image = anchor['image']
        output['augmented'] = self.augmentation_transform(image)
        output['meta'] = anchor['meta']
    ...
    # 'neighbor_top2'
    # 'neighbor_top3'
    # 'neighbor_top1_weight'
    # 'neighbor_top2_weight'
    # 'neighbor_top3_weight'
    # 'possible_neighbors'
    # }